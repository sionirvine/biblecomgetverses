...
(async function main() {
  ...
  try {
    ...
    while (nextChapterButtonFound) {
      ...
      //  -> get info from URL
      console.log(`    scraping URL: ${current_page_url}`);
      const segments = new URL(current_page_url).pathname.split("/");
      const last_part_of_url = segments.pop() || segments.pop(); // Handle potential trailing slash
      if (last_part_of_url) {
        const url_book = last_part_of_url.split(".")[0];

        if (current_usfm_book === "") {
          bookCounter += 1;
          current_usfm_book = url_book;
          result_detail.books_usfm.push(current_usfm_book);
          console.log(`    FIRST book: ${current_usfm_book}`);
        } else {
          if (current_usfm_book != url_book) {
            console.log(`  writing chapter verse count..`);

            // console.log(cv_v_key);
            ...
            console.log(
              `  writing book_verses to output/${VERSION_TO_GET}/${bookCounter}_${current_usfm_book}.json`
            );
            Bun.write(
            ...
            // start the process to get another book
            bookCounter += 1;
            current_usfm_book = url_book;
            result_detail.books_usfm.push(current_usfm_book);
            current_chapter = 0;
            current_chapter_string = "0";
            cv_v_key = {};
            result_verses_per_books = new Array();

            // below line is for testing scrape one book ONLY
            // end = true;

            console.log(`    current book: ${current_usfm_book}`);
          }
        }

        const middle_part = last_part_of_url.split(".")[1];
        if (middle_part.length >= 5) {
          if (middle_part.slice(0, 5) == "INTRO") {
            skip = true;
            console.log(`    INTRO detected, skipping parse..`);
          }
        } else {
          ...
          if (current_chapter == 0) {
            console.log(
              `      NEW chapter: ${current_chapter} (${current_chapter_string})`
            );
          } else {
            console.log(
              `      CURRENT chapter: ${current_chapter} (${current_chapter_string})`
            );
          }
        }
        ...
        try {
          const not_available = await page.waitForSelector(
          ...
          if (not_available) {
            page.goto(
              `https://www.bible.com/bible/${
                BIBLE_VERSION_IDS[`${VERSION_TO_GET}`]
              }/${current_usfm_book}.${current_chapter}`,
            ...
            if (not_available) {
              console.log("content not available. skipping..");
              skip = true;
            }
          }
        } catch (err: any) {
      ...
      if (skip == false) {
        const chapterContainer = await page.waitForSelector(
          `div[data-usfm*='${current_usfm_book}.${current_chapter_string}']`,
          { timeout: 5000 }
        );
