...
(async function main() {
  ...
  try {
    ...
    // Get all book codes by navigating through them
    ...
    for (let i = 0; i < result_detail.books.length; i++) {
      ...
      const bookItem = await page.waitForSelector(
        `div[id^="headlessui-popover-panel-"] > div[class*="overflow-y-auto"] > ul li:nth-child(${
          i + 1
        })`
      );
    ...
    // Create initial tabs
    for (let i = 0; i < Math.min(MAX_CONCURRENT_TABS, bookList.length); i++) {
    ...
    for (let i = 0; i < bookList.length; i += MAX_CONCURRENT_TABS) {
      ...
      console.log(
        `Completed chunk ${Math.floor(i / MAX_CONCURRENT_TABS) + 1}/${Math.ceil(
          bookList.length / MAX_CONCURRENT_TABS
        )}`
      );
    }
