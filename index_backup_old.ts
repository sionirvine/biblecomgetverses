...
class BibleScraperUtils {
   ...
   */
  static async waitTillHTMLRendered(page: Page, timeout = 30000): Promise<void> {
  ...
  static cleanChapterContentClass(chapterContentClass: string): string {
    ...
    const regexChapterContent = /(chaptercontent_)(\w+)(__[a-z0-9]+)(\s)?(.+)?/i;
...
class BibleScraper {
  ...
  constructor(versionKey: string) {
    ...
    this.outputDetailFile = `output/${versionKey}_detail.json`;
    this.outputVersesFile = `output/${versionKey}_verses.json`;
    this.baseUrl = `https://www.bible.com/bible/${this.bibleVersionId}/REV.1.${versionKey}`;
    
    if (!this.bibleVersionId) {
      throw new Error(`Unknown Bible version: ${versionKey}`);
    }
  }

  async initializeOutputDirectory(): Promise<void> {
    await mkdir(`output/${this.versionToGet}`, { recursive: true });
  }
  ...
  private async closeCookiesBanner(page: Page): Promise<void> {
    ...
    try {
      ...
      console.log(`    No cookies banner found or already closed`);
    }
  }
  ...
  private parseBibleInfo(bibInfoArray: (string | null)[], bibleDetail: BibleDetail): void {
    ...
    if (bibInfoArray[languageIndex]) {
      ...
      console.log(`        BIBLE LANG: ${bibleDetail.language}`);
    }

    // Extract name and abbreviation
    if (bibInfoArray[versionIndex]) {
      const filteredVersion = bibInfoArray[versionIndex]!
        .replace("Version: ", "")
        .split("-");
      
      bibleDetail.name = filteredVersion[0].trim();
      bibleDetail.abbreviation = filteredVersion[1]
        ?.trim()
        .replaceAll(" ", "")
        .replace("神", "SHEN")
        .replace("上帝", "SHANGDI")
      ...
      console.log(`        BIBLE NAME: ${bibleDetail.name}`);
      console.log(`        BIBLE ABBV: ${bibleDetail.abbreviation}`);
    }
  }

  async scrapeAllBooks(): Promise<void> {
    console.log(`Starting scrape process for Bible ${this.versionToGet}..`);
    ...
    try {
      ...
      console.log(`  Opening page ${this.baseUrl}`);
